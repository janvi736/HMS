...
if (isset($_GET['action']) && $_GET['action'] == 'patient_cancel') {
  $id = mysqli_real_escape_string($conn, $_GET['id']);
  $patient_id = $_SESSION['user_id'];
  mysqli_query($conn, "UPDATE appointments SET status='cancelled' WHERE id='$id' AND patient_id='$patient_id'");
  header("Location: /hospital-management/pages/patient/book_appointment.php?success=Appointment cancelled successfully!");
  exit();
}
?>
